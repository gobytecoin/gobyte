...
"""Script for verifying GoByte Core release binaries

This script attempts to download the signature file SHA256SUMS.asc from
github.com and gobytecore-binaries.gobyte.network and compares them.
...
WORKINGDIR = "/tmp/gobyte_verify_binaries"
HASHFILE = "hashes.tmp"
HOST1="https://github.com/gobytecoin/gobyte/releases/download/v"
HOST2="https://gobytecore-binaries.gobyte.network/file/gobytecore-binaries/"
VERSIONPREFIX=""
...
def main(args):
    ...
    if not success:
        print("github.com failed to provide signature file, "
              "but gobytecore-binaries.gobyte.network did?")
        print("wget output:")
        print(indent(output, '\t'))
    ...
    if not files_are_equal(sigfile1, sigfile2):
        print("github.com and gobytecore-binaries.gobyte.network signature files were not equal?")
    ...
    if retval != 0:
        if retval == 1:
            ...
            print("gpg error. Do you have the GoByte Core binary release "
    ...
    for _, binary_filename in hashes_to_verify:
        ...
        if not success:
            print(f"Error: failed to download {binary_filename}")
            print(indent(output, '\t'))
            return 2

        signature_file = binary_filename + ".asc"
        success, output = download_with_wget(HOST1 + remote_dir + signature_file)
        if not success:
            print(f"Error: failed to download {signature_file}")
            print(indent(output, '\t'))
            return 2
        ...
        if retval != 0:
            if retval == 1:
                ...
                print("gpg error. Do you have the GoByte Core binary release signing key installed?")
            print("gpg output:")
            print(indent(output, '\t'))
    ...
    if len(args) >= 2:
        ...
        remove_files([sigfile1, sigfile2] + verified_binaries + asc_files)
    else:
