...
ARG USER_ID=1000 \
    GROUP_ID=1000
RUN set -ex; \
    (getent group ${GROUP_ID} && usermod -g ${GROUP_ID} ubuntu) || groupmod -g ${GROUP_ID} -n gobyte ubuntu; \
    usermod -u ${USER_ID} -md /home/gobyte -l gobyte ubuntu; \
