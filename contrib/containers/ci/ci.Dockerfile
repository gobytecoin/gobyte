...
# clang-3.7.1 (ubuntu-14.04 vintage, used by native_cctools) carries a hard
# NEEDED entry for libtinfo.so.5. Ubuntu 24.04 Noble ships only libtinfo.so.6.
# Create a compat symlink so the toolchain binary loads correctly.
RUN ln -s /usr/lib/x86_64-linux-gnu/libtinfo.so.6 \
