# Bootstrappable GoByte Core Builds

This directory contains the files necessary to perform bootstrappable GoByte
Core builds.
...
To build GoByte Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      GoByte Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/gobyte-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/gobytecoin/guix.sigs).
