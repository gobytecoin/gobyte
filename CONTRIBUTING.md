Contributing to GoByte Core
============================

The GoByte Core project operates an open contributor model where anyone is
welcome to contribute towards development in the form of peer review, testing
and patches. This document explains the practical process and guidelines for
contributing.

First, in terms of structure, there is no particular concept of "GoByte Core
...
Before you start contributing, familiarize yourself with the GoByte Core build
system and tests. Refer to the documentation in the repository on how to build
GoByte Core and how to run the unit and functional tests.

There are many open issues of varying difficulty waiting to be fixed.
If you're looking for somewhere to start contributing, check out the
[good first issue](https://github.com/gobytecoin/gobyte/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22)
list.
...
Most communication about GoByte Core development happens on Discord Server.
...
typical types, the `backport` type should be used for dash backport PRs. For
...
include:
  ...
  - *interfaces* for changes to the GUI
...
fork of GoByte Core copies the merge. Instead, make any username mentions in a
subsequent comment to the PR.

### Translation changes

Note that translations should not be submitted as pull requests. Please see
[Translation Process](https://github.com/gobytecoin/gobyte/blob/master/doc/translation_process.md)
...
When a pull request conflicts with the target branch, you may be asked to rebase it on top of the current target branch.
    ...
    git fetch https://github.com/gobytecoin/gobyte  # Fetch the latest upstream commit
...
The following applies to code changes to the GoByte Core project (and related
projects such as libsecp256k1), and is not to be confused with overall GoByte
Network Protocol consensus changes.

Whether a pull request is merged into GoByte Core rests with the project merge
maintainers.
...
Patches that change GoByte consensus rules are considerably more involved than
...
Where a patch set proposes to change the GoByte consensus, it must have been
...
The review process is normally fairly responsive on the GoByte Core repository, however
...
Dash Backports are an incredibly valuable part of GoByte's development. Backporting allows us to easily implement new
features, improvements and fixes as dash implements them.

To see detailed statistics & progress see Google Sheet tracker: [Dash backports for GoByte](https://docs.google.com/spreadsheets/d/#TODO).
You should use this sheet to find what PRs to backport and its commit.

Updating the Spreadsheet
-------------------
To keep the spreadsheet up to date we need to pull merges made to Dash for each version.

### Adding Dash Remote

If you have not already, add the dash repo as a remote:

```
git remote add dash git@github.com:dashpay/dash.git
```

This allows you to easily cherry-pick merges and look into logs of dash without switching directories.

### Pulling Merges

To pull the most up-to-date merges first make sure dash is up-to-date:
...
```
git log --first-parent --oneline dash/<version_start>..dash/<version_end> >> <filename>.txt
```
...
```
git log --first-parent --oneline dash/0.14..dash/0.15 >> backports_0.14-0.15.txt
```

will pull all merges made to Dash version 0.14 until the start of version 15 and write to `backports_0.14-0.15.txt`.
...
```
git log --first-parent --oneline dash/<cur_ver>..dash/master >> <filename>.txt
```

this will pull all the merges made to Dash since the release of the current version.
