...
#
# NodeConn: an object which manages p2p connectivity to a gobyte node
...
MY_VERSION = 70209  # MIN_PEER_PROTO_VERSION
...
def gobytehash(s):
    return gobyte_hash.getPoWHash(s)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(gobytehash(r))
            self.hash = encode(gobytehash(r)[::-1], 'hex_codec').decode('ascii')
