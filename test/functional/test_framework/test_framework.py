...
class BitcoinTestFramework():
    ...
    def stop_node(self, i, wait=0):
        """Stop a gobyted test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple gobyted test nodes"""
        for node in self.nodes:
...
class GoByteTestFramework(BitcoinTestFramework):
    def set_gobyte_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
