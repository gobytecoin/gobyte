...
// which are part of the GoByte repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="gobyte-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /gobyte-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "gobytecoin-gobyte/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from gobytecoin-gobyte/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'gobytecoin-gobyte/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from gobytecoin-gobyte/develop"
            }
          }

          if (hasCache) {
            sh "cd /gobyte-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /gobyte-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /gobyte-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /gobyte-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /gobyte-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /gobyte-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /gobyte-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
