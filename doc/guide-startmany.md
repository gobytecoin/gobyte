...
2. [Sending 1000 GBX to new wallet addresses.](#option2)
...
### Send 1000 GBX to New Addresses

Just like setting up a standard MN. Send exactly 1000 GBX to each new address created above.
...
[From the gobyte github repo](https://github.com/gobytecoin/gobyte/blob/master/doc/masternode_conf.md)
...
If you are using a `masternode.conf` file you no longer need the `gobyte.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `gobyte.conf` file. This option should be used only to start local Hot masternode now.

## Update gobyte.conf on server

If you generated a new masternode private key, you will need to update the remote `gobyte.conf` files.
...
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `gobyte.conf` file.
...
and comparing with the official explorer at https://explorer.gobyte.org/chain/GoByte
