...
- [Development guidelines](#development-guidelines)
    - [General GoByte Core](#general-gobyte-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
GoByte Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while gobyted is running with the `logging`
RPC.  For instance, launching gobyted with `-debug` or `-debug=1` will turn on
...
GoByte Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run gobyted.

You can then use the `-debug=lock` configuration option at gobyted startup or
`gobyte-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `gobyte-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/gobytecoin/gobyte/blob/master/contrib/valgrind.supp))
...
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_gobyte --log_level=test_suite
...
# A coverage report will now be accessible at `./test_gobyte.coverage/index.html`.
```
...
To profile a running gobyted process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep gobyted` -- sleep 60
```
...
GoByte Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`gobyted`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as GoByte Core, where everyone uses
...
pay attention to for reviewers of GoByte Core code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: GoByte Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of GoByte Core, in which case
...
a GoByte Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of GoByte Core, the wallet was always
...
- Use *invalid* GoByte addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
