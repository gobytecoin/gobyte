...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GOBYTETE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    nTimeGoByteSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - GoByte specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeGoByteSpecific * MICRO, nTimeGoByteSpecific * MILLI / nBlocksTotal);
