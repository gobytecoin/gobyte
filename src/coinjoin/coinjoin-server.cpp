...
void CCoinJoinServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        if (pfrom->nVersion < MIN_COINJOIN_PEER_PROTO_VERSION) {
            ...
            if (enable_bip61) {
                connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, strprintf("Version must be %d or greater", MIN_COINJOIN_PEER_PROTO_VERSION)));
            }
...
/*
    ...
    allow endless transaction that would bloat GoByte and make it unusable. To
