...
*
*   Why is this needed?
*   - In GoByte some blocks are superblocks, which output much higher amounts of coins
...
int CMasternodePayments::GetMinMasternodePaymentsProto() {
    if(IsSporkActive(SPORK_15_STRICT_NEW_PROTO_FLAG)) return MIN_MASTERNODE_PAYMENT_PROTO_VERSION_3;
    
    if(IsSporkActive(SPORK_10_MASTERNODE_PAY_UPDATED_NODES)) return MIN_MASTERNODE_PAYMENT_PROTO_VERSION_2;
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f GBX\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
