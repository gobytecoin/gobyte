...
namespace CoinJoin {
//! Interface for the wallet constrained src/coinjoin part of a gobyte node (gobyted process).
class Client
{
public:
    virtual ~Client() {}
...
struct WalletAddress {
    ...
    WalletAddress(CTxDestination dest, isminetype is_mine, std::string name, std::string purpose) : dest(std::move(dest)), is_mine(is_mine), name(std::move(name)), purpose(std::move(purpose))
    {
    }
};
