...
void MacNotificationHandler::showNotification(const QString &title,
  ...
  if (this->hasUserNotificationCenterSupport()) {
    ...
    [notificationCenterInstance performSelector:@selector(deliverNotification:)
...
MacNotificationHandler *MacNotificationHandler::instance() {
  ...
  if (!s_instance) {
    ...
    if (aPossibleClass) {
      // change NSBundle -bundleIdentifier method to return a correct bundle
      // identifier a bundle identifier is required to use OSXs User
      // Notification Center
      method_exchangeImplementations(
          class_getInstanceMethod(aPossibleClass, @selector(bundleIdentifier)),
