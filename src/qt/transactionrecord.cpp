    ...
    if (nNet > 0 || wtx.is_coinbase) {
        ...
        if (wtx.is_denominate) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                sub.idx = parts.size();
                if (wtx.tx->vin.size() == 1 && wtx.tx->vout.size() == 1 && coinJoinOptions.isCollateralAmount(nDebit) && coinJoinOptions.isCollateralAmount(nCredit) && coinJoinOptions.isCollateralAmount(-nNet)) {
            ...
            bool fDone = false;
            if (wtx.tx->vin.size() == 1 && wtx.tx->vout.size() == 1 && coinJoinOptions.isCollateralAmount(nDebit) && nCredit == 0 // OP_RETURN
...
{
    return status.cur_num_blocks != numBlocks || status.needsUpdate || (!status.lockedByChainLocks && status.cachedChainLockHeight != chainLockHeight);
}
