 ...
 *  We have 4 times smaller block times in GoByte, so we need to push 4 times more invs per 1MB. */
...
void PushNodeVersion(CNode* pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, nProtocolVersion, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe, nonce, strSubVersion, nNodeStartingHeight, ::fRelayTxes, mnauthChallenge, pnode->m_masternode_connection));
...
PeerLogicValidation::PeerLogicValidation(CConnman* connmanIn, CScheduler& scheduler, bool enable_bip61) : connman(connmanIn), m_stale_tip_check_time(0), m_enable_bip61(enable_bip61)
{
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && !pfrom->fWhitelisted && ((((pfrom->GetLocalServices() & NODE_NETWORK_LIMITED) == NODE_NETWORK_LIMITED) && ((pfrom->GetLocalServices() & NODE_NETWORK) != NODE_NETWORK) && (chainActive.Tip()->nHeight - pindex->nHeight > (int)NODE_NETWORK_LIMITED_MIN_BLOCKS + 2 /* add two blocks buffer extension for possible races */)))) {
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // GOBYTE this code also handles non-TXs (GoByte specific messages)
